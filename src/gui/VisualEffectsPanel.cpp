    ...
    : QWidget(parent), viewport_(viewport)
{
    ...
    // Logical sequence: Light → Shadow → Fog → Blur → SSAO. It runs from what
    // lights the scene, through the scene itself, to the passes that
    // post-process the finished image.
    //
    // Floor used to sit between Shadow and Fog here — it moved to the
    // Spatial References dock (gui::FloorPanel), since it draws a reference
    // surface IN the scene rather than an image filter, the same role the
    // cell wireframe and axes triad play there.
    //
