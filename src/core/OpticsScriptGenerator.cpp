...
std::string generateVaspOpticsScript(const OpticsConfig& cfg)
{
    ...
    out << "# The restart needs the density and wavefunctions on disk,\n"
           ...
           "except Exception:\n"
           "    nbands_scf = 0\n"
           "# Occupied bands from the electron count where VASP reports it;\n"
           "# the SCF's own NBANDS (occupied + VASP's small buffer) is the\n"
           "# fallback estimate.\n"
           "try:\n"
           "    n_occ = int(np.ceil(\n"
        ...
        << "    # LOPTICS sums transitions into empty states, and VASP's\n"
           "    # default NBANDS barely covers occupation. Empty bands as the\n"
           "    # configured percentage of the occupied count (100 % = as\n"
           "    # many empty as occupied), floored so a tiny cell cannot\n"
           "    # truncate the transition sum to nothing.\n"
        << "    n_empty = max(int(round(n_occ * " << cfg.emptyBandsPercent
        << " / 100.0)), 12)\n"
           "    params[\"nbands\"] = max(n_occ + n_empty, nbands_scf)\n"
           "    print(f\"CALANGO_INFO bands occupied={n_occ} \"\n"
    ...
    out << "atoms.calc = Vasp(**params)\n"
        ...
        << "    \"algo\": \"" << algo << "\",\n"
        << "    \"empty_bands_percent\": " << cfg.emptyBandsPercent << ",\n"
...
std::string generateOpticsScript(const OpticsConfig& cfg)
{
    ...
    out << "# Optical properties (linear dielectric response) — generated by "
        ...
        << "# Empty bands as the configured percentage of the occupied count\n"
           "# (100 % = as many empty as occupied), floored so a tiny cell\n"
           "# cannot truncate the transition sum to nothing.\n"
           "n_empty = max(int(round(n_occ * "
        << cfg.emptyBandsPercent
        << " / 100.0)), 12)\n"
           "n_bands = n_occ + n_empty\n"
           "print(f\"CALANGO_INFO bands occupied={n_occ} empty={n_empty}\",\n"
           "      flush=True)\n"
