...
std::string generateElfScript(const ElfConfig& cfg)
{
    ...
    const std::string compute =
        "elf_grid = None\n"
        "_errs = []\n"
        "# GPAW 25/26 replaced the ELF class with a free function. Both the\n"
        "# `ELF` class and `calc.get_electronic_localization_function` are gone\n"
        "# from 25.7 AND 26.7, so the legacy path below is dead on every\n"
        "# currently shipping GPAW and is kept only for older installs.\n"
        "try:\n"
        "    from gpaw.elf import elf_from_dft_calculation\n"
        "\n"
        "    elf_grid = elf_from_dft_calculation(calc)\n"
        "except Exception as _e:\n"
        "    _errs.append('gpaw.elf.elf_from_dft_calculation: ' + repr(_e))\n"
        "    try:\n"
        ...
        "        _errs.append('gpaw.elf.ELF: ' + repr(_e2))\n"
        "        try:\n"
        "            elf_grid = calc.get_electronic_localization_function()\n"
        "        except Exception as _e3:\n"
        "            _errs.append('calc.get_electronic_localization_function: '\n"
        "                         + repr(_e3))\n"
        "# The modern call returns a UGArray wrapper, not a bare ndarray.\n"
        "elf_grid = getattr(elf_grid, 'data', elf_grid)\n"
        "if elf_grid is None:\n"
