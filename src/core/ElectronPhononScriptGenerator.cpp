...
std::string generateElectronPhononScript(const ElectronPhononConfig& cfg)
{
    ...
    out << "#!/usr/bin/env python3\n"
        ...
        << "),\n"
           ...
           "#\n"
           "# NOTE on the mass convention, which is an OPEN question: GPAW\n"
           "# divides by sqrt(2 * (amu/m_e) * omega), i.e. by ONE atomic mass\n"
           "# unit, on the assumption that ASE's mass-weighted eigenvectors\n"
           "# already carry the atomic mass. GPAW's own source flags this\n"
           "# ('# potential BUG: M needs to be unit cell mass according to some\n"
           "# sources'). If the atomic mass belongs here instead, |g|^2 for\n"
           "# aluminium would be smaller by a factor of 27 — which is the right\n"
           "# order to explain the lambda this benchmark reports. NOT YET\n"
           "# RESOLVED; do not change without checking ASE's normalization.\n"
