...
void emitTask(std::ostringstream& out, const CalculatorConfig& c)
{
    ...
    case TaskKind::MolecularDynamics: {
        ...
        // The seven ASE integrators, emitted by the shared block in
        // core/MdIntegratorBlocks.hpp. Shared rather than spelled out here
        // because the thermodynamic-integration windows have to propagate
        // the SAME dynamics with the SAME coupling parameters — two copies
        // of this table would let an MD run and a TI window disagree about
        // what "thermostat coupling" means, silently.
        md_blocks::emitIntegrator(out, c);
