...
{
    std::ostringstream out;
    out << "# Wannier-interpolated band structure — generated by Calango\n"
           "import json\n"
           "import os\n"
           "import glob\n"
           "import numpy as np\n"
        ...
        << "_gpw = sorted(glob.glob(os.path.join(_base, '*.gpw')))\n"
           "if not _gpw:\n"
           "    raise RuntimeError('No GPAW wavefunction (.gpw) found in the MLWF "
           "job '\n"
           "                       'directory ' + _base + '. Re-run the MLWF "
           "setup so '\n"
           "                       'it saves the wavefunctions.')\n"
           "from gpaw import GPAW\n"
           "calc = GPAW(_gpw[0], txt='gpaw_wannier_bands.txt')\n"
           "atoms = calc.get_atoms()\n"
           "try:\n"
           ...
           "    efermi = 0.0\n"
           "_log.progress(1, 3)\n"
           "# Rebuild the same localization the MLWF run produced (its .gpw does\n"
           "# not carry the Wannier rotation), reading n and the trial\n"
           "# projection back from wannier.json.\n"
           "_meta = {}\n"
           "_mj = os.path.join(_base, 'wannier.json')\n"
           "if os.path.exists(_mj):\n"
           "    _meta = json.load(open(_mj))\n"
           "nwannier = len(_meta.get('centers', [])) or 4\n"
           ...
           "    _prev = _val\n"
           "_log.progress(2, 3)\n"
           "# Interpolate eigenvalues along an automatic band path from the\n"
           ...
           "    energies.append(np.sort(_eigs.real))\n"
           "energies = np.asarray(energies)[None, :, :]  # (nspin=1, nk, nbands)\n"
           "from ase.spectrum.band_structure import BandStructure\n"
           "bs = BandStructure(path=path, energies=energies, reference=efermi)\n"
           ...
           "    json.dump(data, _fh)\n"
           "_log.progress(3, 3)\n"
           "print('CALANGO_RESULT bands=bands.json', flush=True)\n";
    return out.str();
}
    ...
    const std::string& mlwfDir, const WannierInterpolationConfig& cfg)
{
    ...
    out << "# Wannier-interpolated bands + PDOS — generated by Calango\n"
        ...
        << "_gpw = sorted(glob.glob(os.path.join(_base, '*.gpw')))\n"
           ...
           "                       'directory ' + _base + '.')\n"
           "from gpaw import GPAW\n"
           "calc = GPAW(_gpw[0], txt='gpaw_wannier_interp.txt')\n"
           "atoms = calc.get_atoms()\n"
           "try:\n"
           "    efermi = float(calc.get_fermi_level())\n"
           "except Exception:\n"
