...
\vfill
{\large July 2026 --- Calango 26.7.23\par}
\vspace*{1.2cm}
\end{titlepage}
...
\file{calango-26.7.23-macos-arm64.dmg} \\
Linux & \opt{DEB} & \file{calango\_26.7.23\_amd64.deb} \\
\bottomrule
\end{tabular}
\end{center}
...
The result is \file{build-dmg/calango-26.7.23-macos-arm64.dmg}. Mounting
...
\begin{lstlisting}
APP=_CPack_Packages/Darwin/DragNDrop/calango-26.7.23-macos-arm64/calango.app
...
hdiutil attach -readonly -nobrowse calango-26.7.23-macos-arm64.dmg
\end{lstlisting}
...
The result is \file{calango\_26.7.23\_<arch>.deb}. Install it via
\opt{apt} rather than \opt{dpkg -i}, so the dependencies are resolved:

\begin{lstlisting}
sudo apt install ./calango_26.7.23_amd64.deb
\end{lstlisting}
...
\begin{lstlisting}
dpkg-deb --info  calango_26.7.23_amd64.deb   # control fields, Depends
dpkg-deb --contents calango_26.7.23_amd64.deb
...
Symptom & Cause / fix \\
\midrule
\texttt{CPack Error: Cannot find description file name:
[/opt/homebrew/.../CPack.GenericDescription.txt]} &
\opt{cpack} was run in a build directory configured on macOS (a
Dropbox-synced \file{build/} tree). Configure a fresh \file{build-deb}
with the Linux \file{cmake}; never reuse a cross-platform build tree. \\
\addlinespace
\texttt{file INSTALL cannot find
".../assets/calango/icon\_transparent.svg"} &
A stale install rule referenced a vector icon that is not in the repo.
The Linux install now ships \file{icon\_transparent.png} to
\file{/usr/share/pixmaps}; add a
\file{hicolor/scalable/apps/calango.svg} install only if a real SVG is
committed. \\
\addlinespace
\texttt{dpkg-shlibdeps: error: cannot find library} during
\opt{cpack} &
The binary links a non-system \file{libpython} (Conda/Homebrew on
\texttt{\$PATH}). Configure with
\opt{-DPython3\_EXECUTABLE=/usr/bin/python3}. The build sets
\opt{PYBIND11\_FINDPYTHON=ON} so pybind11 reuses the \file{FindPython3}
interpreter instead of its legacy \file{PATH} search; check
\lstinline|readelf -d build-deb/calango| shows
\file{libpython3.x.so.1.0} from \file{/usr/lib}, not from
\file{\$HOME}. \\
\bottomrule
\end{tabular}
\end{center}
