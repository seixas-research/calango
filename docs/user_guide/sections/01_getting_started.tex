...
\texttt{mace-torch} (ML potentials), \texttt{gpaw} (DFT bands and PDOS),
\texttt{icet} (optional, improves SQS generation). A complete environment:
\begin{lstlisting}
python3 -m venv .venv
...
Once a structure is loaded, \kbd{F} frames it in the viewport and the
\ui{Structure} panel fills in with formula, cell parameters and symmetry.
