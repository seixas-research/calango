...
\begin{description}
  ...
  \item[\ui{Spatial References}] Four tabs: the cell wireframe, the ground
    plane, the orientation triad, and the per-atom vector overlay. All four
    draw something onto the scene that is not the atoms.
    ...
    Under \ui{Floor}, \ui{Height} raises or lowers the ground plane relative
    to its automatic position with a linked slider and spin box --- the
    slider's own range scales with the structure's size, and the box beside
    it accepts anything further out than the slider currently reaches. It
    moved here from \ui{Visual Effects}, where it sat as its own tab, because
    the plane is a reference surface drawn in the scene, the role the cell
    wireframe and the triad also play, not a post-process image filter.
