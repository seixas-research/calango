...
Both take their input from the same row: open an \file{\_hr.dat}, or load one
...
That feature generates a Python script and runs it against a completed DFT
baseline; this one evaluates the same quantity in process from
$H(\mathbf{R})$. They answer the same question from different inputs, which
makes them a cross-check rather than duplicates --- and this panel reports the
Chern number twice, once from the curvature integral and once from the centre
winding, so that a disagreement flags under-sampled $k$.
