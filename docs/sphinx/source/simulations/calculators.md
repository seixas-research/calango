...
## Calango DFTB — native, tight binding

Calango's **own** Slater-Koster (SCC-)DFTB implementation (`src/dftb`), reading the same `.skf` parameter format as DFTB+ above with its own parser. It does **not** link or shell out to DFTB+ or any other tight-binding package: the Hamiltonian is built and diagonalized natively in C++, dispatched through a small native binary (`calango-dftb-run`, installed beside the app) — a thin, self-contained generated Python script writes the structure and a plain-text task manifest and execs it as a subprocess, relaying its output line for line, so it goes through the same job/result pipeline every scripted engine above does.

Shares the **same** "DFTB settings" group as DFTB+ above (Slater-Koster directory, SCC toggle, SCC tolerance, max SCC iterations, filling temperature) and the shared Brillouin-zone k-grid — the two engines read the identical file format the identical way, so one settings group serves both.

**Level of theory.** Non-SCC (DFTB0, a single Hamiltonian diagonalization) when SCC is off; SCC (DFTB2 — self-consistent Mulliken charges through the Elstner et al. (1998) γ-functional second-order term, its long-range part lattice-summed by Ewald for a periodic structure) when on, which is the default whenever the loaded parameter set provides Hubbard U values (every mainstream set does). DFTB3 and spin-polarized DFTB are not implemented.

**Basis.** *s* and *p* orbitals only — a `.skf` file's *d*-shell columns are parsed (so a heavier-element file still round-trips exactly) but never assembled into a Hamiltonian; a structure whose parameter set needs *d* orbitals is out of scope for now. Every mainstream light/medium-element set (mio, 3ob, pbc, matsci) is *s*, *p* for its non-metal elements.

**Forces are by finite difference** of the total energy, not analytic — following the same precedent the native all-electron DFT engine's own `ForceCalculator` already set in this codebase (a hand-differentiated Slater-Koster angular transform is a real source of sign/algebra bugs; six extra Hamiltonian solves per atom is not, for a single point). This is *why* the engine is offered for **Single-Point only** — a geometry optimization or MD run would each re-pay that 6×N cost per step — and why it never offers **Run Remote**: the native binary ships beside this application, not on a cluster.

**Electronic-structure post-processing.** The engine itself computes Bands, PDOS (Mulliken/species/shell-projected), Effective Band Structure (Popescu-Zunger unfolding — the S-weighted LCAO projection this basis needs, not a plane-wave one), and Optics (independent-particle Kubo-Greenwood/RPA, no local-field effects, with the same per-area 2D observables the GPAW/VASP Optics module reports for a sheet) — see `tests/dftb_native_run_test.py`, `tests/dftb_unfolding_run_test.py` and `calango_dftb_test` for how each is validated against closed-form references and a real end-to-end run. **The dedicated wizards for these features (Electronic Structure, Effective Bands, 2D Bands, Optics) do not yet offer this engine** — each currently assumes a mandatory prior SCF baseline from GPAW/VASP/QE/SIESTA, an architectural assumption this self-contained engine does not fit, and extending it is `FUTURE.md` work. Single-point energies, forces and Mulliken charges are available from the Single-Point wizard today.

**Parameter sets are not bundled.** Like DFTB+'s own Slater-Koster directory above, `.skf` files are data with their own redistribution terms — download the set you need from [dftb.org](https://dftb.org) (mio, 3ob, pbc, matsci, …) and check its license before using it, exactly as you would a VASP POTCAR library or a set of Wannier Hamiltonians.
