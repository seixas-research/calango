...
- `phonon_modes.json` — complex eigenvectors at the path's named high-symmetry points, for the Vibrational Analysis mode animation (irrep labels at Γ only — away from the zone center the factor group is not the little group of q).
