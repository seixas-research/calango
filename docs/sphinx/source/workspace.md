...
- **Spatial References** — four tabs, {guilabel}`Unit cell`, {guilabel}`Floor`, {guilabel}`Axes triad` and {guilabel}`Vectors`: everything that answers "where/which way is this?" about the scene without being the atoms. Under {guilabel}`Unit cell`, *Show atoms of the neighboring unit cell* draws exactly the periodic images that terminate a bond leaving the cell — only those, so no bond ends in mid-air. {guilabel}`Floor` is covered in {doc}`/viewport`, alongside the rest of the scene furniture.
