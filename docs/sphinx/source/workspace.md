...
- **Spatial References** — four tabs, {guilabel}`Unit cell`, {guilabel}`Axes triad`, {guilabel}`Vectors` and {guilabel}`Floor` (last, since it draws a whole extra object into the scene rather than an overlay on the structure the way the other three do): everything that answers "where/which way is this?" about the scene without being the atoms. Under {guilabel}`Unit cell`, *Show atoms of the neighboring unit cell* draws exactly the periodic images that terminate a bond leaving the cell — only those, so no bond ends in mid-air. {guilabel}`Floor` is covered in {doc}`/viewport`, alongside the rest of the scene furniture.
...
The process manager. Each row keeps a live link to its job directory; double-clicking loads the result — except a {guilabel}`GO/MCMD` row, which opens that run's {ref}`MCMD Summary <go-mcmd-summary>` window instead, live or finished.
```
