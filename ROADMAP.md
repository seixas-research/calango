...
*   [x] Brand asset integration (v0.13.1): high-resolution lizard icon set under `assets/calango/` (PNG + SVG variants) — `icon_white.png` embedded as the application window/taskbar/dock icon (`QApplication::setWindowIcon`), `icon_transparent.png` as the About Calango banner, theme-aware logo in the README, and an opt-in `-DCALANGO_MACOS_BUNDLE=ON` CMake path (bundle metadata + .icns hook) that leaves the plain `./build/calango` developer binary untouched by default.
*   [x] Job-panel plot polish (v0.13.1): labeled axes on both charts (rotated "Total Energy (eV)" / "Temperature (K)" y-labels, step x-label with live readout) and an Export Data… button per tab writing the recorded series as `.csv`/`.dat` — energy as (step, total_energy_eV), temperature as (step, temperature_K, target_K with the thermostat setpoint when one exists).
*   [x] Axes triad drawn at double stroke weight (v0.13.1): the corner triad moved from 1-device-pixel GL_LINES (core profile clamps glLineWidth, so it could never be thickened and looked hairline on retina) to antialiased QPainter strokes at ~2× visual weight, DPI-scaled, for both Cartesian and lattice-vector modes; the dead GL axes program was removed.
